        ...
        when (action.toActionType()) {
            ...
            ActionType.SEND_SMS -> {
                if (call.hasArgument(MESSAGE_BODY)
                    && call.hasArgument(ADDRESS) && call.hasArgument(SIMSLOT)
                ) {
    ...
    private fun handleGetActions(smsAction: SmsAction) {
        smsController.apply {
            val value: Any = when (smsAction) {
                SmsAction.IS_SMS_CAPABLE -> isSmsCapable(simSlot)
                SmsAction.GET_CELLULAR_DATA_STATE -> getCellularDataState(simSlot)
                SmsAction.GET_CALL_STATE -> getCallState(simSlot)
                SmsAction.GET_DATA_ACTIVITY -> getDataActivity(simSlot)
                SmsAction.GET_NETWORK_OPERATOR -> getNetworkOperator(simSlot)
                SmsAction.GET_NETWORK_OPERATOR_NAME -> getNetworkOperatorName(simSlot)
                SmsAction.GET_DATA_NETWORK_TYPE -> getDataNetworkType(simSlot)
                SmsAction.GET_PHONE_TYPE -> getPhoneType(simSlot)
                SmsAction.GET_SIM_OPERATOR -> getSimOperator(simSlot)
                SmsAction.GET_SIM_OPERATOR_NAME -> getSimOperatorName(simSlot)
                SmsAction.GET_SIM_STATE -> getSimState(simSlot)
                SmsAction.IS_NETWORK_ROAMING -> isNetworkRoaming(simSlot)
                SmsAction.GET_SIGNAL_STRENGTH -> {
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                        getSignalStrength(simSlot)
